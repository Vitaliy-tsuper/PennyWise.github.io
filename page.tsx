...
export default function Home() {
  ...
  const fetchTransactions = useCallback(async () => {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        // Фільтруємо транзакції для поточного користувача, якщо він є
  ...
  useEffect(() => {
    if (currentUser) {
      fetchTransactions();
    } else if (!authLoading) { // Тільки якщо перевірка автентифікації завершена
      setTransactions([]);
      setLoading(false);
    }
  ...
  const handleAddTransaction = async (transactionData: Omit<Transaction, 'id' | 'userEmail'>) => {
    if (!currentUser || !currentUser.email) {
     ...
     const dataToSend = {
       ...
       userEmail: currentUser.email, // Додаємо email користувача
     };

    const result = await dbAddTransaction(dataToSend as any); // TODO: Fix type for action
    ...
    if (result && 'id' in result) {
      ...
      // Оновлюємо список транзакцій, додаючи нову та сортуючи
   ...
   const handleDeleteTransaction = async (id: string | number) => {
     ...
     try {
      const result = await dbDeleteTransaction(numericId);
  ...
  const calculateBalance = () => {
    ...
    // Баланс розраховується тільки для транзакцій поточного користувача
  ...
  // Фільтруємо доходи та витрати для поточного користувача
